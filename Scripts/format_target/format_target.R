...
# Modified by Lars Fritsche to not Switch from PLINK2 to PLINK1.9 and back to PLINK2 to avoid information loss.
start.time <- Sys.time()
library("optparse")
...
# Extract variants based on new reference RSIDs and create final output
system(paste0(opt$plink2, " --pfile ", tmp_dir, "/subset --extract ", tmp_dir, "/extract_list_2.txt --make-pgen --threads 1 --out ", opt$output))
