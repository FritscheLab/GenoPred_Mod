...
# Modified to generate raw, unscaled reference PGS without ancestry adjustment.
start.time <- Sys.time()
library("optparse")
...
opt$output <- paste0(outdir, "/reference/pgs_score_files_raw/ref_scoring_raw")
...
log_header(log_file = log_file, opt = opt, script = "ref_scoring_raw.R", start.time = start.time)
...
if (!is.null(score_files)) {
    ...
    for (i in 1:nrow(score_files)) {
        # Use AllAncestry for raw PGS output path
        pgs_i <- paste0(outdir, "/reference/pgs_score_files_raw/", score_files$method[i], "/", score_files$name[i], "/ref-", score_files$name[i], "-AllAncestry.profiles")
...
if (is.null(score_files) || nrow(score_files) == 0) {
    log_add(log_file = log_file, message = paste0("No score files to be used for raw reference scoring."))
    end.time <- Sys.time()
...
# We will process score files and perform reference scoring for one chromosome for efficiency
for (chr_i in CHROMS) {
...
###
# Output raw, unscaled polygenic scores
###

log_add(log_file = log_file, message = paste0("Saving raw, unscaled reference PGS."))

# Create output directories for each score file
for (i in 1:nrow(score_files)) {
    output_dir <- paste0(outdir, "/reference/pgs_score_files_raw/", score_files$method[i], "/", score_files$name[i])
    ...
    fwrite(scores_i, output_i, sep = " ", na = "NA", quote = F)

    log_add(log_file = log_file, message = paste0("Raw reference PGS saved: ", output_i))
}

log_add(log_file = log_file, message = paste0("Raw reference scoring completed for ", nrow(score_files), " score files."))
